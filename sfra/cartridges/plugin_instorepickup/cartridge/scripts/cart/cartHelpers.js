...
function createInStorePickupShipmentForLineItem(basket, storeId, req) {
    ...
    if (basket && storeId) {
        ...
        if (!shipment) {
            ...
            var store = JSON.parse(req.form.storeInfo);
            var storeAddress = {
                address: {
                    ...
                    city: store.city,
                    stateCode: store.additionalServiceCode,
...
function addProductToCart(currentBasket, productId, quantity, childProducts, options, storeId, req) {
    ...
    if (productInCart) {
        ...
        if (productLineItem.product.custom.availableForInStorePickup) {
            if (storeId) {
                instorePickupStoreHelper.setStoreInProductLineItem(storeId, productLineItem, req);
            }
        }
